...
module cdf
  ...
  use constants, only : ir, zero, one, stdout, stderr
  use netcdf
  implicit none
  private
  ...
  interface cdfvarput
     module procedure cdfvarr4put,cdfvarr3put,cdfvarr2put,cdfvarr1put,cdfvarr0put
  ...
  interface cdfcopy
     module procedure cdfcopyr4,cdfcopyr3,cdfcopyr2,cdfcopyr1,cdfcopyr0
...
contains
  ...
  !**************************************
  ! write a time dependent variable of real kind (4d)
  subroutine cdfvarr4put(name,val,dims,flip,time,slice, &
       units,long_name,missing_value)
    implicit none
    
    ! in
    character(len=*), intent(in) :: name
    real (kind=ir), intent(in) :: val(:,:,:,:)
    character(len=*), intent(in) :: dims(:)
    logical, intent(in), optional :: flip
    real (kind=ir), intent(in), optional :: time
    integer, optional :: slice(:,:)
    character(len=*), intent(in), optional :: units
    character(len=*), intent(in), optional :: long_name    
    real (kind=ir), intent(in),optional :: missing_value

    ! local (dimension dependent)
    character (len=11), parameter :: subname='cdfvarr4put'
    real (kind=ir), allocatable :: r(:,:,:,:,:)
    ! local (rest)
    ...
    if (nds.gt.0) then
       ...
       call cdferror(.not.cdfcheckdim(llen(1:nds),rlen(1:nds),flip=lflip,slice=slice), &
            subname//': space dimensions in file and variable not consistent :'//name )
    end if
    ...
    if (ierr.eq.0) then
       ...
       call cdferr(nf90_def_var(cdf_ncid,name,nf90_double,dimids(1:nd),varid))
    ...
    ! allocate memory
    allocate(r(lcount(1),lcount(2),lcount(3),lcount(4),1),stat=ierr)
  ...
  subroutine cdfvarr3get(name,val,flip,time,tindex,slice, &
    ...
    character(len=*), intent(in) :: name
    logical, intent(in), optional :: flip
    real (kind=ir), optional :: time
    integer, optional :: tindex
    integer, optional :: slice(:,:)

    ! out
    real (kind=ir), intent(out) :: val(:,:,:)
    character(len=*), intent(out), optional :: units
    character(len=*), intent(out), optional :: long_name    
    real (kind=ir), intent(out),optional :: missing_value
    ...
    integer :: i,ierr,it,nd,nds
    integer :: lstart(nf90_max_dims),lcount(nf90_max_dims),lstride(nf90_max_dims)
    real (kind=ir) :: add_offset, scale_factor
    logical :: lflip
  ...
  subroutine cdftime(dtime,dtimeu)
    ...
    if (ierr.ne.0) then
       ...
       dimid(1)=cdf_dtimeid
       call cdferr(nf90_def_var(cdf_ncid,cdf_dtime,nf90_double,dimid,tvid))
       call cdferr(nf90_enddef(cdf_ncid))
  ...
  subroutine cdfbegin(fname,caller,create,overwrite,readonly)
    ...
    if (present(create)) then
       if (create) then
          ...
          call cdferr(nf90_enddef(ncid,512*1024)) ! add some header space
  ...
  subroutine cdftimeput(it,t)
    ...
    if (nt.gt.0) then
       ...
       do it=1,nt
          if (abs(ts(it)-t).lt.0.0001_ir) exit
       end do
       if (it.gt.nt) then
  ...
  !**************************************
  ! copy and flip 4d field (if necessary)
  subroutine cdfcopyr4(dst,src,flip)
    implicit none

    ! in
    real (kind=ir), intent(in) :: src(:,:,:,:)
    logical, intent(in), optional :: flip

    ! out
    real (kind=ir), intent(out) :: dst(:,:,:,:)
       ...
       if (flip) then
          do i=slb(1),sub(1)
             do j=slb(2),sub(2)
                do k=slb(3),sub(3)
                   do l=slb(4),sub(4)
                      dst(l-slb(4)+dlb(4),k-slb(3)+dlb(3),j-slb(2)+dlb(2),i-slb(1)+dlb(1))= &
                        src(i,j,k,l)
                   end do
                end do
             end do
    ...
    call cdfdebug('cdfcopyr4: exit')

  end subroutine cdfcopyr4
